...
reset_right_timer = time.perf_counter()
is_right_gesture = False
threshold = 2

#info for virtual cam
_, frame = cap.read()
x, y, c = frame.shape
framerate = cap.get(cv2.CAP_PROP_FPS)

# initialize keyboard contoller
keyboard = Controller()
with pyvirtualcam.Camera(width=y, height=x, fps=framerate, backend = 'unitycapture') as cam:
    ...
    while True:
        ...
        #resets position tracking if right hand has been out of the scene for over 0.5s
        if not is_right and time.perf_counter() - reset_right_timer > 0.5:
            last_pos = [0,0]
    ...
    ##           timer = time.perf_counter()
        ...
        #creates a virtual cam for other programs to use
        cam.send(framergb)
